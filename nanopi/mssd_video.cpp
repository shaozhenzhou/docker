...
int main(int argc, char *argv[])
{
    ...
    int res;
    while( ( res=getopt(argc,argv,"p:m:f:h"))!= -1)
    {
        switch(res)
        {
            case 'p':
            ...
            case 'h':
                std::cout << "[Usage]: " << argv[0] << " [-h]\n"
                          << "   [-p proto_file] [-m model_file] [-f video_file]\n";
                return 0;
            default:
                break;
        }
    ...
    VideoWriter writer;
    writer.open(video_file.substr(0, video_file.rfind(".")) + "-proc.mp4", CV_FOURCC('m', 'p', '4', 'v'), 30, size, true);
    ...
    while(capture.read(frame)){
        ...
        std::cout << "--------------------------------------\n";
        std::cout << "time takes is " << total_time << " ms\n";
